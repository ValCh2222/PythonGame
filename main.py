...
class Lava(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        for num in range(1, 4):
            ...
            pygame.transform.scale(imag, (tile_size, tile_size))
...
class Player():
    ...
    def update(self, game_over):
        ...
        if game_over == 0:
            ...
            if (key[pygame.K_SPACE] or key[
                pygame.K_w]) and self.jumped == False and self.number_of_jumpes > 0 and self.rect.y > 0 and self.rect.y < screen_height:
                self.vel_y = -13
                self.number_of_jumpes -= 1
            ...
            if (key[pygame.K_RIGHT] or key[pygame.K_d]) and self.rect.x < 1150:
                ...
                self.direction = 1
            if (key[pygame.K_LEFT] == False and key[pygame.K_RIGHT] == False) or (
            ...
            if pygame.sprite.spritecollide(self, blade_group, False) or pygame.sprite.spritecollide(self, blade2_group,
                                                                                                    False):
                if check_lives():
                    self.reset(100, 700)
                else:
            ...
            draw_text('GAME OVER!', font, (255, 25, 255), (screen_width // 2) - 200, screen_height // 2)
            if self.rect.y > 200:
                self.rect.y -= 5
    ...
    def reset(self, x, y):
        ...
        for num in range(1, 6):
            img_right = pygame.image.load(f'images/skins/character{skin}{num}.png')
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        for num in range(1, 9):
            img_right = pygame.image.load(f'images/enemies/enemy1{num}.png')
        ...
        for row in data:
            ...
            for tile in row:
                ...
                if tile == '7':
                    ...
                    coin_group.add(coin)
                if tile == 'l':
                    item_mes = Button(col_count * tile_size + (tile_size // 2),
                ...
                if tile == 'i':
                    ball = Star(col_count * tile_size + (tile_size // 2), row_count * tile_size + (tile_size // 2),
                                star_image)
...
info_button = Button(screen_width // 2 + 300, screen_height // 2, infobtn_img)
...
while run:
    ...
    if main_menu == True and list_of_items == False:
        ...
        screen.blit(info_img, (screen_width // 2 - 300, screen_height // 2 - 300))
